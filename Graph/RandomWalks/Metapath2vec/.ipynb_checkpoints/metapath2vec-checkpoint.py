...
class Metapath2vec(torch.nn.Module):
    ...
    def __init__(self, edge_index_dict, embedding_dim, metapath, walk_length,
        ...
        assert metapath[0][0] == metapath[-1][-1] # metapath는 symmetric
        ...
        types = set([x[0] for x in metapath]) | set(x[-1] for x in metapath) # node type
        types = sroted(list(types))
    ...
    # metapath의 첫번째 type의 node 갯수만큼을 불러오고(모든 노드에서 한번씩 randomwalk을 해야하므로)
    # collate_fn을 이용해서 batch를 나눔
    ...
    def pos_sample(self, batch):
        ...
        batch = batch.repeat(self.walks_per_node) # node하나당 시작되는 randomwalk 횟수

        rws = [batch] # randomwalk start 약자인듯?
    ...
    # batch argument는 collate_fn 함수 api에 지정되어 있음    
